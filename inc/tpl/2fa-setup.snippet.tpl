...
<div class="sucuri-2fa-info">
    <div id="two-factor-info" class="sucuriscan-2fa-info-box">
     <div id="two-factor-info-deactivated">
        ...
        <p class="sucuri-with-icon"><strong>{{Two-factor authentication is not activated.}}</strong><br>{{Please scan the QR, or input the key manually in your preferred 2FA app, and verify to activate it.}}<br><br>{{Please keep in mind that activating this option for all users will block them from being able to log-in until they activate their two-factor authentication. Each user will be prompted to activate upon their next login attempt.}}</p>
        <ol class="circle-numbers">
            <li>{{Open your authenticator app on your phone.}}</li>
            <li>{{Scan the QR code on the screen (or enter the key manually).}}</li>
            <li>{{Enter the 6-digit code generated by the app.}}</li>
            <li>{{Click "Setup" to activate.}}</li>
        </ol>
     </div>
